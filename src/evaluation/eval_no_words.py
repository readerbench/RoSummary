...
def _worker(items: list[dict]) -> Tuple[List[Any], List[int]]:
    ...
    for item in tqdm(items):
        ...
        words = [token.text for token in model_ro(item['generate']) if not token.is_punct]

        no_words_original.append(item['no-words'])
        no_words_generate.append(len(words))

    return no_words_original, no_words_generate


def eval_no_words_items(items: dict) -> dict:
    no_words_original = []
    no_words_generate = []
...
def eval_all_generate_no_words(path_director: str) -> None:
    results = dict()
    ...
    for path_director in tqdm(Path(path_director).glob("**/sample_output/")):
        ...
        for path in tqdm(Path(path_director).glob("*.json")):
            ...
            if 'no-words' not in data[0]:
                continue

            results_director[path.name.replace('.json', '')] = eval_no_words_items(data)
        if not results_director:
            continue

        results[path_director.parent.name] = results_director

    with open('result_no_words.json', 'w+') as file_output:
        json.dump(results, file_output, ensure_ascii=False, indent=4)


if __name__ == '__main__':
    eval_all_generate_no_words("/home/mihai/Desktop/backup/genaration/genaration")
